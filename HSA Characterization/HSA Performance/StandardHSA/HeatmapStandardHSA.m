...
figure()
subplot(1, 3, 1)
surfc(thetaMat, dispMat, fMeanMat, 'EdgeAlpha', 0.5)
colorbar;
clim([-2e4 2e4]);
title({'Standard HSA';'Mean Force'}, 'FontSize', 10)
view(0, 90)
xticks(tl)
...
subplot(1, 3, 2)
surfc(thetaMat, dispMat, tauMeanMat, 'EdgeAlpha', 0.5)
colorbar;
clim([-50 65]);
title({'Standard HSA';'Mean Torque'}, 'FontSize', 10)
view(0, 90)
xticks(tl)
xticklabels(ticklabels)
ylabel('Extension [mm]')
...
subplot(1, 3, 3)
surfc(rotMatrix, dispMat, kFitMatrix, 'EdgeAlpha', 0.5);

...
%{
    ...
    surface plots to generate a heatmap for force, torque, and spring constant (average) for the Stacked HSA.
%}
...
clim([-15 30]);
title({'Stacked HSA';'Mean Force (F)'}, 'FontSize', 10)
view(0, 90)
xticks(tl)
...
set(title_handle ,'String', title_string);
clim([-60 75]);
title({'Stacked HSA';'Mean Torque (\tau)'}, 'FontSize', 10)
view(0, 90)
xticks(tl)
xticklabels(ticklabels)
ylabel('Extension [mm]')
...
set(title_handle ,'String', title_string);
clim([0 5]);
title({'Stacked HSA';'Mean Spring Constant (k)'}, 'FontSize', 10);
view(0, 90)
xticks(tl)
xticklabels(ticklabels)
ylabel('Extension [mm]')
